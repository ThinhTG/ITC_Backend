...
namespace ITC.Services.Revenue
{
	public class RevenueDashboardService : IRevenueDashboardService
    {
		...
		public async Task<RevenueDashboardDto> GetDashboardAsync(DateTime? from, DateTime? to)
		{
			var transactions = await _transactionRepo.GetAllAsync(); // Includes Wallet
			var query = transactions.AsQueryable();
			...
			var subscriptionRevenue = filtered
				.Where(t => t.TransactionType == "subscription")
				.Sum(t => t.Amount);

			var monthlySubscriptionRevenue = filtered
				.Where(t => t.TransactionType == "subscription" &&
			...
			var totalTransactions = filtered.Count;
			var avgValue = totalTransactions > 0 ? subscriptionRevenue / totalTransactions : 0;
			...
			var revenueByCategory = filtered
				...
				.Select(g => new RevenueByCategoryDto
				{
					Category = g.Key,
					Amount = g.Sum(x => x.Amount),
					Percent = subscriptionRevenue > 0 ? Math.Round((double)g.Sum(x => x.Amount) * 100 / (double)subscriptionRevenue, 1) : 0
				})
				.ToList();

			// --- Recent Transactions with Customers ---
			var recentTransactionsWithUsers = new List<RecentTransactionDto>();
			var recentTransactions = filtered
				.OrderByDescending(t => t.TransactionDate)
				.Take(10)
			...
			var completedWithdrawals = withdrawalQuery
				.Where(w => w.Status == ITC.Core.Enum.WithdrawalStatus.Completed)
				.ToList();
			...
			var recentWithdrawalsWithUsers = completedWithdrawals
				.OrderByDescending(w => w.RequestDate)
				.Take(10)
				.Select(w => new RecentWithdrawalDto
				{
					Customer = w.Account?.FullName ?? w.Account?.UserName ?? "Unknown",
					Date = w.RequestDate.DateTime,
					Amount = w.Amount,
					BankName = w.BankName,
			...
			var jobsWithFees = jobQuery
				.Where(j => j.PlatformServiceFee.HasValue && j.PlatformServiceFee > 0)
				.ToList();
			...
			var recentPlatformFeesWithDetails = jobsWithFees
				.OrderByDescending(j => j.CreatedAt)
				.Take(10)
				.Select(j => new RecentPlatformFeeDto
				{
					JobTitle = j.JobTitle,
					Customer = j.Customer?.FullName ?? j.Customer?.UserName ?? "Unknown",
					Date = j.CreatedAt.DateTime,
					Amount = j.PlatformServiceFee ?? 0,
					JobType = j.TranslationType
				})
				.ToList();

			// --- Final Revenue ---
			var totalRevenue = subscriptionRevenue + totalPlatformFees;
			var monthlyRevenue = monthlySubscriptionRevenue + monthlyPlatformFees;
